                                                ...
                                                struct sid_demo_action_notification *action_notify)
{
    ...
    tl.tag = SID_DEMO_TAG_RAK_SIDEWALK_DATA_NOTIFY;
    tl.len = sizeof(sidewalk_payload_t);
    sid_write_entry_tlv_nbytes(state, &tl, (const uint8_t *const) &(action_notify->rak_sidewalk_payload), sizeof(sidewalk_payload_t));
                                          ...
                                          struct sid_demo_action_notification *action_notify)
{
    ...
    while (state->offset < state->buffer_len) {
        ...
        switch (tl.tag) {
            ...
            //     break;
            case SID_DEMO_TAG_RAK_SIDEWALK_DATA_NOTIFY:
                sid_read_entry_nbytes(state,(uint8_t *const) &(action_notify->rak_sidewalk_payload),  sizeof(sidewalk_payload_t));
                break;
            case SID_DEMO_TAG_CURRENT_GPS_TIME_IN_SECONDS:
