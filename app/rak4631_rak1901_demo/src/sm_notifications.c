...
void sm_notify_sensor_data(app_context_t *app_context, bool button_pressed, sidewalk_payload_t sidewalk_payload)
{
	assert(app_context);
	...
	memcpy(&(action_notify.rak_sidewalk_payload), &sidewalk_payload, sizeof(sidewalk_payload_t));

	LOG_INF("RAK Sidewalk Payload Size: %d", sizeof(sidewalk_payload_t));
	LOG_HEXDUMP_INF(&(action_notify.rak_sidewalk_payload), sizeof(sidewalk_payload_t), "RAK Sidewalk Payload:");
	...
	LOG_INF("Sidewalk Message Size: %d", msg.size);
	LOG_HEXDUMP_INF(msg.data, msg.size, "Sidewalk Message:");
