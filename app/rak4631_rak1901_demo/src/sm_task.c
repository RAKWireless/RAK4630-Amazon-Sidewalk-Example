...
#include <sm_task.h>
// #include <sm_leds.h> // Edited by Sercan ERAT
// #include <sm_buttons.h> // Edited by Sercan ERAT
#include <sm_timers.h>
...
static int32_t sid_stop_link(app_context_t *context, struct sid_config *config)
{
	sid_error_t ret = sid_stop(context->sidewalk_handle, config->link_mask);
	if (ret != SID_ERROR_NONE) {
		LOG_ERR("Failed to stop sidewalk, err:%d", (int)ret);
		(void)sid_deinit(context->sidewalk_handle);
		context->sidewalk_handle = NULL;
		config->link_mask = 0;
		return -1;
...
static int32_t sid_start_link(app_context_t *context, struct sid_config *config)
{
	sid_error_t ret = sid_start(context->sidewalk_handle, config->link_mask);
	if (ret != SID_ERROR_NONE) {
		LOG_ERR("Failed to start sidewalk, err:%d", (int)ret);
		(void)sid_deinit(context->sidewalk_handle);
		context->sidewalk_handle = NULL;
...
static void set_device_profile(struct sid_handle *sid_handle)
{
	...
	struct sid_device_profile curr_dev_cfg = {
		.unicast_params = {
			.device_profile_id = SID_LINK3_PROFILE_A,
		},
	};
	...
	if (ret == SID_ERROR_NONE) {
		...
		if (!memcmp(&curr_dev_cfg, &target_dev_cfg, sizeof(curr_dev_cfg))) {
			LOG_INF("Device profile for Link type 3 already set");
		} else {
			ret = sid_option(sid_handle, SID_OPTION_900MHZ_SET_DEVICE_PROFILE,
					 &target_dev_cfg, sizeof(target_dev_cfg));
			if (ret != SID_ERROR_NONE) {
				LOG_ERR("Device profile configuration for Link type 3 failed ret = %d",
					ret);
				sm_device_profile_timer_set_and_run(
					K_MSEC(CONFIG_SM_TIMER_PROFILE_CHECK_TIMER_DELAY_MS));
			} else {
				LOG_INF("Device profile Link type 3 set success");
			}
		}
	} else {
		LOG_ERR("Failed to get device profile configuration for Link type 3 ret = %d", ret);
		sm_device_profile_timer_set_and_run(
			K_MSEC(CONFIG_SM_TIMER_PROFILE_CHECK_TIMER_DELAY_MS));
	}
}
	...
	while (1) {
		...
		if (!k_msgq_get(&sm_main_task_msgq, &event, K_FOREVER)) {
			...
			case EVENT_FACTORY_RESET:
				...
				break;
			case EVENT_STOP_LINK:
            {
				LOG_INF("Stop link...");
				sid_stop_link(app_context, &config);
				//sm_task_suspend();
				break;
            }
			case EVENT_START_LINK:
				LOG_INF("Start link...");
				sid_start_link(app_context, &config);
				break;
			case EVENT_BUTTON_PRESS:
					LOG_INF("EVENT_BUTTON_PRESS");
				//sm_notify_sensor_data(app_context, true);
				// sm_buttons_pendig_flag_clear(); // Edited by Sercan ERAT
				break;
			case EVENT_NOTIFICATION_TIMER_FIRED:
					LOG_INF("EVENT_NOTIFICATION_TIMER_FIRED");
				...
				// } 
					...
					// bt_le_adv_stop(); // This is disabling ble adv for LP TESTS... 	
					
					/* Send Sensor DATA*/
					sidewalk_payload_t payload = rak_sidewalk_get_payload();
					sm_notify_sensor_data(app_context, false, payload);
				}
				break;
			case EVENT_BUTTON_PRESS_TIMER_FIRED:
					LOG_INF("EVENT_BUTTON_PRESS_TIMER_FIRED");
				if (app_context->sidewalk_state != STATE_SIDEWALK_READY) {
					if (BUILT_IN_LM == SID_LINK_TYPE_1) {
						if (!(app_context->link_status.link_mask &
						      SID_LINK_TYPE_1) &&
			...
			case EVENT_SET_DEVICE_PROFILE_LINK_TYPE_2:
					LOG_INF("EVENT_SET_DEVICE_PROFILE_LINK_TYPE_2");
				if (BUILT_IN_LM == SID_LINK_TYPE_3) {
				...
				if (msg_desc.status_hdr_ind &&
					...
					if (msg_desc.cmd_id ==
						...
						sid_parse_state_init(&state, msg.payload,
								     msg.payload_size);
						//sm_buttons_action_response_process(&state); // Edited by Sercan ERAT
					}
					...
					//sm_leds_action_request_process(&g_app_context, &state); // Edited by Sercan ERAT
				}
			}
		}
	}
...
void sm_task_stop_link(void)
{
	enum event_type event = EVENT_STOP_LINK;
	sm_main_task_msg_q_write(event);
}

void sm_task_start_link(void)
{
	enum event_type event = EVENT_START_LINK;
	sm_main_task_msg_q_write(event);
}
